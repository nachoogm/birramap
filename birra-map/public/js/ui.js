...
const UI = (() => {
  ...
  /* Las teselas las gestiona TEMA: cambian solas al cambiar de tema */
  function capaMapa() {
    if (typeof TEMA !== 'undefined' && TEMA.capaMapa) return TEMA.capaMapa();
  ...
  /* Esqueleto: se ve la forma de la lista mientras carga.
     Se percibe mucho más rápido que un spinner en medio. */
  const esqueleto = (n = 4) => Array.from({ length: n }, () => `
    <div class="skel-item">
      <div class="skel skel-ico"></div>
      <div style="flex:1"><div class="skel skel-l1"></div><div class="skel skel-l2"></div></div>
      <div class="skel skel-fin"></div>
    </div>`).join('');

  return {
    esc, estrellas, pickerEstrellas, reparto, TEXTOS_NOTA,
    pedirUbicacionUI, selectorMapa, capaMapa,
    tour, PASOS, AYUDA, pintarAyuda, toast, vacio, cargando, esqueleto
  };
})();
