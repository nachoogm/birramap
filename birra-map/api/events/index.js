...
/* GET  /api/events            → eventos del grupo (Oktoberfest, feria, despedida…)
   POST /api/events            → crea uno { name, startsAt, endsAt }
   DELETE /api/events?id=..    → lo borra (solo quien lo creó) */
module.exports = S.withMember(async (context, req, p, m) => {
  ...
  if (req.method === 'POST') {
    ...
    if (endsMs <= startsMs) return S.err(context, 400, 'El final tiene que ser posterior al inicio');
    ...
    await S.addEvent(m.groupId, {
      id, name, startsMs, endsMs, createdBy: p.userId, createdByNick: m.nick,
  ...
  if (req.method === 'DELETE') {
    ...
    if (ev.createdBy !== p.userId) return S.err(context, 403, 'Solo lo puede borrar quien lo creó');
    const t = await S.tbl(S.T.events);
    await t.deleteEntity(ev.partitionKey, ev.rowKey);
    return S.ok(context, { ok: true });
  }

  const list = (await S.listEvents(m.groupId)).map(e => ({
    id: e.id, name: e.name, startsMs: e.startsMs, endsMs: e.endsMs,
    startsAt: e.startsAt, endsAt: e.endsAt, createdByNick: e.createdByNick,
    active: now >= e.startsMs && now <= e.endsMs,
    upcoming: now < e.startsMs
  }));
  S.ok(context, list);
});
