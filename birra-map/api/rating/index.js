...
/* POST /api/rating  → puntúa un bar de 0 a 5
   body: { place, stars, note? }
   Cada persona tiene un voto por bar: si vuelves a votar, se sustituye. */
module.exports = S.withMember(async (context, req, p, m) => {
  ...
  await S.addRating(m.groupId, {
    userId: p.userId, nick: m.nick,
    place, placeKey: R.clavePlace(place),
    stars, note: String(b.note || '').slice(0, 120),
    ...
    ts: new Date(tsMs).toISOString(), tsMs,
    day: new Date(tsMs).toISOString().slice(0, 10)
  });

  /* devuelve cómo queda el bar tras tu voto */
  ...
  S.ok(context, {
    ok: true, place, stars,
    bar: bar ? { place: bar.place, media: bar.media, votos: bar.votos, etiqueta: R.etiqueta(bar.media, bar.votos) } : null
  }, 201);
});
