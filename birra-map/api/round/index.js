...
/* POST /api/round → "invito yo", con el precio unitario que pongas
   GET  /api/round?days=90 → histórico */
module.exports = S.withMember(async (context, req, p, m) => {
  if (req.method === 'GET') {
    ...
    return S.ok(context, rounds.map(r => ({
      id: r.rowKey, payerId: r.payerId, payerNick: r.payerNick, drink: r.drink, place: r.place,
      ts: r.ts, tsMs: r.tsMs, priceCents: r.priceCents || 0, size: r.size,
  ...
  await S.addRound(m.groupId, {
    payerId: p.userId, payerNick: m.nick, drink, place, priceCents,
  ...
  S.ok(context, {
    ok: true, participants, size: participants.length, priceCents,
    totalCents: priceCents * participants.length
  }, 201);
});
