...
module.exports = S.withMember(async (context, req, p, m) => {
  ...
  S.ok(context, { days, totalRounds:rounds.length, debts, balance:L.roundStats(rounds),
    mine: { owes: debts.filter(d => d.fromId === p.userId), owed: debts.filter(d => d.toId === p.userId) } });
});
